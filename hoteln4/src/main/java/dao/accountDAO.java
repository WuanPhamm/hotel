...
public class accountDAO implements daoInterface<account> {
    ...
    public int insert(account t) {
        ...
        try {
            ...
            if(kq == 1) {
            	if(t.getRole_id() == 1) {
            		user u = new user(0, t.getAcc_username(), null, "", 0, "", "", account_id);
            		
            		kq = userDAO.getInstance().insert(u);
            	}else if(t.getRole_id() == 2) {
            		customer cus = new customer(0, t.getAcc_username(), t.getAcc_username(), null, 0, "", "", "", "", 0, account_id);
    ...
    public ArrayList<account> selectByCondition(String condition) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                acc.setAcc_username(rs.getString("account_username"));
                acc.setAcc_password(rs.getString("account_password"));
                acc.setAcc_status(rs.getInt("account_status"));
                acc.setAcc_created_at(rs.getTimestamp("account_created_at"));
                acc.setAcc_updated_at(rs.getTimestamp("account_updated_at"));
                acc.setAcc_last_login_at(rs.getTimestamp("account_last_login_at"));
                acc.setAcc_login_time(rs.getInt("account_login_time"));
                acc.setRole_id(rs.getInt("role_id"));
                accounts.add(acc);
            }
        } catch (Exception e) {
